...
public class DaoPostImp implements DaoPost {
	...
	public ArrayList<Post> getPosts(String clubId) {
		...
		try {
			...
			ArrayList<Post> postsOfClub = (ArrayList<Post>) session
					.createSQLQuery("SELECT * FROM post WHERE club_clubId='" + clubId + "' ORDER BY publishedDate DESC")
	...
	public ArrayList<Post> getAllPosts() {
		...
		try {
			@SuppressWarnings("unchecked")
			ArrayList<Post> postsOfClub = (ArrayList<Post>) session.createSQLQuery("SELECT * FROM post")
					.addEntity(Post.class).list();
			session.getTransaction().commit();
	...
	public Post delete(Post post) {
		...
		try {
			...
			@SuppressWarnings("unchecked")
			ArrayList<Post> postList = (ArrayList<Post>) session
					.createSQLQuery("SELECT * FROM post WHERE id='" + post.getId() + "'").addEntity(Post.class).list();

			session.delete(postList.get(0));
			session.getTransaction().commit();
			return post;
		} catch (Exception e) {
			session.getTransaction().rollback();
