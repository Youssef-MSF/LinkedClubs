...
<body>
	...
	<form id="container" method="post" action=""
		...
		<input type="text"
			value="<c:out value="${sessionScope.student.CNE}"/>" name="CNE"
			hidden />
		<h2>Setting your profile</h2>
		<span></span>
		<div>
			<label for="email">Email:</label> <input type="email" id="email"
				name="email" placeholder="Type your new Email"
				value="${sessionScope.student.email}"> <span class="err">${err.get("errEmail")}</span>
		</div>
		<div class="wrapper">
			<div>
			...
			<div>
				<label for="password2">Confirmation:</label> <input type="password"
		...
		<div>
			<div>
				<label for="Instagram">Instagram:</label> <input type="text"
					id="Instagram" name="instagram" placeholder="Type your instagram"
					value="${sessionScope.student.instagram}"> <span
					class="err">${err.get("errInstagram")}</span>
			</div>
			<div>
				<label for="Facebook">Facebook:</label> <input type="text"
					id="Facebook" name="facebook" placeholder="Type your facebook"
					value="${sessionScope.student.facebook}"> <span class="err">${err.get("errFacebook")}</span>
			</div>
		</div>
		<div class="wrapper">
			<div>
			...
			<div>
				<label for="level">Major:</label> <select name="major" id="level"
					required>
		...
		<div class="wrapper">
			<div>
				<label for="profileImage">Profile Image:</label> <input type="file"
					name="profileImage"
					value="<c:out value="${sessionScope.student.profileImage}"/>"
					id="profileImage"> <span class="err">${err.get("errProfileImage")}</span>
			</div>
			<div>
				<label for="profileImage">Background Image:</label> <input
					type="file" name="backgroundImage"
					value="<c:out value="${sessionScope.student.coverImage}"/>"
					id="profileImage"> <span class="err">${err.get("errCoverImage") }</span>
			</div>
		</div>
		<div>
			<label for="Bio">Bio:</label>
			<textarea name="bio" id="Bio" cols="30" rows="2"
				placeholder="Type your bio"> <c:out
					value="${sessionScope.student.studentBio}" /></textarea>
			<span class="err">${err.get("errbio")}</span>
		</div>
		<div>
			<input id="submit" style="margin: 15px 0px;" type="submit"
				value="Submit"> <input id="reset" type="reset" value="Reset">
		</div>
	</form>
</body>
</html>